...
public class TeamManager {
	...
	/*
	 * public void createTeams() { int layer = 0; boolean noTeam; for(Entry<Player,
	 * PlayerData> entry : this.playerData.entrySet()) {
	 * if(entry.getValue().getTeam().equals(Team.NONE)) { noTeam = true;
	 * while(noTeam) { for(Team team : TEAM_ORDER.subList(0, this.teamCount - 1)) {
	 * if(this.getTeam(team).size() == layer) { entry.getValue().setTeam(team);
	 * noTeam = false; break; } } layer++; } } } }
	 */
