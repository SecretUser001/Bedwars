...
public class Game {
	...
	public void breakBed(Team team) {
		...
		for(Player player : this.teamManager.getPlayer()) {
			player.sendMessage(ChatColor.BOLD + "" + ChatColor.WHITE + "BED DESTRUCTION > " + (this.teamManager.getTeam(team).contains(player) ? (ChatColor.GRAY + "Your bed ") : (team.getPrefix()) + " Bed ") + ChatColor.GRAY + "was destroyed");
		}
